...
(define (expnt x y) (apply-generic 'expnt x y))
...
(define (install-ploy-package)
  ...
  (define (pseudoremainder-terms L1 L2)
    (let ((factor (expnt (coeff (first-term L2))
                         (make-integer
                          (+ 1
                            (highest-order L1)
                            (- (highest-order L2)))))))
  ...
  (define (gcd-terms L1 L2)
    (if (empty-termlist? L2)
        (reduce-common-factor L1)
        (gcd-terms L2 (pseudoremainder-terms L1 L2))))
  (define (common-factor L)
    (define (iter factor terms)
      (if (empty-termlist? terms)
          factor
          (iter (greatest-common-divisor factor
                                         (coeff (first-term terms)))
                (rest-terms terms))))
    (iter (coeff (first-term L)) (rest-terms L)))
  (define (reduce-common-factor L)
    (if (empty-termlist? L)
        (the-empty-termlist-sparse)
        (let ((factor (common-factor L)))
          (car (div-terms L
                          (adjoin-term (make-term 0 factor)
                                       (the-empty-termlist-sparse)))))))
  (define (reduce-terms-with f L)
    (if (empty-termlist? L)
        (the-empty-termlist-sparse)
        (car (div-terms L
                        (adjoin-term (make-term 0 f)
                                     (the-empty-termlist-sparse))))))
  (define (reduce-terms n d)
    (let ((g (gcd-terms n d)))
      (let ((factor (expnt (coeff (first-term g))
                           (make-integer
                            (+ 1
                               (max (highest-order n)
                                    (highest-order d))
                               (- (highest-order g)))))))
        (let ((f*n (mul-term-by-all-terms (make-term 0 factor) n))
              (f*d (mul-term-by-all-terms (make-term 0 factor) d)))
          (let ((nn (car (div-terms f*n g)))
                (dd (car (div-terms f*d g))))
            (let ((ff (greatest-common-divisor (common-factor nn)
                                               (common-factor dd))))
              (list (reduce-terms-with ff nn)
                    (reduce-terms-with ff dd))))))))
  (define (neg-terms L)
    (if (empty-termlist? L)
        (the-empty-termlist-sparse)
        (let ((t (first-term L)))
  ...
  (define (reduce-poly p1 p2)
    (let ((v (major-variable p1 p2)))
      (if v
          (let ((nndd (reduce-terms (term-list p1) (term-list p2))))
            (list (make-poly v (car nndd))
                  (make-poly v (cadr nndd))))
...
(define int make-integer)

(define p1 (make-polynomial 'x (list 'dense (int 1) (int 1))))
(define p2 (make-polynomial 'x (list 'dense (int 1) (int 0) (int 0) (int -1))))
(define p3 (make-polynomial 'x (list 'dense (int 1) (int 0))))
(define p4 (make-polynomial 'x (list 'dense (int 1) (int 0) (int -1))))

(define rf1 (make-rational p1 p2))
(define rf2 (make-rational p3 p4))

(test car (contents (add rf1 rf2)))
(test cdr (contents (add rf1 rf2)))
