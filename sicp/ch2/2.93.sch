...
(define p1 (make-polynomial 'x '(sparse (2 1.0) (0 1.0))))
(define p2 (make-polynomial 'x '(sparse (3 1.0) (0 1.0))))
...
(define r1 (make-rational (make-integer 12)
                          (make-integer 5)))
(define r2 (make-rational (make-integer 13)
                          (make-integer 7)))
...
(test mul r1 (make-rational 1 2))

;letting rational package handles polynomial numerator and denominator impose
;serious problem. if we dont attempt to modify the types tower structure, that
;is, add a new type "rational polynomial", we must add some constraints.
;consider the following case, if we add a real number to a rational polynomial,
;what should be done? since the type level of real number is higher than
;rational polynomial, should we convert rational polynomial to "real polynomial"
;? that does not make sense. thus we would not implemet mixed type operation if
;any operand is rational polynomial. let is, if any operand is rational
;polynomial, the other must be rational number or polynomial.
;
;except for changing primitive operations to generic operations, there are some
;important point to take care. first, since coerce rational polynomial does not
;make sense, we modify the rasie and project procedure to return zero if either
;the numerator or the denominator is polynomial. doing so avoid drop procedure
;to do coercion on rational polynomial. on the other side, there is no way to
;avoid other procedure from raising the type of rational polynomial (= 0), this
;is why we make the constriant on mixed type operation.
;
;another point is how to modify equ-rat? in the original version, it simply
;divide the numerator and denominator, then do comparision. now we cannot
;use generic div procedure, since it would casue two problems:
;
;  1. the drop procedure first project the number, than raise it back to check
;     if the converted number has the same value. this cause the following loop:
;     (drop rat) <-----------
;     (equ? rat int)        |
;     (equ? rat rat)        |
;     (and (div int int)    | ----> unstoppable!
;          (div int int))   |
;     (and (drop rat) -------
;          (div int int))
;
;  2. divide polynomial results in two terms, we need to write aux procedure
;     to do comparasion.
;
;thus we choose to multiply numerator with the other denominator, then do
;comparision, doing so avoids the above problem.
